...
def build_digest(req: DigestRequest) -> DigestResponse:
    ...
    if not matches:
        return build_mock_digest(
            req, 
            notes=["No feeds configured for these regions/topics. Showing mock demo data."]
        )
    ...
    if not all_candidates:
        return build_mock_digest(
            req, 
            notes=["No recent articles found in feeds. Showing mock demo data."]
        )
    ...
    for _i, can in enumerate(all_candidates[:req.max_cards]):
        ...
        cards.append(DigestCard(
            ...
            bullets=[
                Bullet(text=bullet_text, citations=[citation])
            ],
            sources=[citation]
        ))
